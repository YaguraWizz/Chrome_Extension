...
chrome.runtime.onMessage.addListener(function (message) {
  var groupedPagesList = document.getElementById("grouped-pages");
  var ungroupedPagesList = document.getElementById("ungrouped-pages");

  // Выводим страницы в группе
  message.groupedPages.forEach(function (page) {
    var li = document.createElement("li");
    li.textContent = page;
    groupedPagesList.appendChild(li);
  });
 
  // Выводим страницы без группы
  message.ungroupedPages.forEach(function (page) {
    var li = document.createElement("li");
    li.textContent = page;
    ungroupedPagesList.appendChild(li);
  });
});

// Запрос данных у background.js
        ...
        function: (color) => {
          ...
          if (color.trim() === "") {
            ...
            if (backupStyle) {
              body.setAttribute(
                "style",
                `${backupStyle}; background-color: ${color} !important;`
              );
            } else {
              body.setAttribute(
                "style",
